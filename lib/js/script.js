...
function login(token){
    window.history.pushState('', 'najajason', '/najajason');
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                if(g_stopAt > g_currentAt){
                    ...
                    if(!cashedOut && betting){
                        ...
                        placeBet( parseFloat(g_stopAt), parseFloat(g_bet), parseFloat(g_currentAt+0.00001), parseFloat( ( g_stake * g_currentAt ) ) );
                        return;
                    }else{
