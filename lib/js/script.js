...
$(function(){
    ...
    $('#autoBet_button').click(function(){
        if(!user.connected){
            window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1493&response_type=token&state=Meh&redirect_uri=https://najajason.github.io/najajason";
            return;
        }
        if(AutoRunning){
            AutoRunning = false;
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                if(g_stopAt > g_currentAt){
                    ...
                    if(!cashedOut && betting){
                        ...
                        placeBet( parseFloat(g_stopAt), parseFloat(g_bet), parseFloat(g_currentAt+0.0002), parseFloat( g_stake + g_bet ) );
                        return;
                    }else{
