...
$(function(){
    ...
    $('#autoBet_button').click(function(){
        if(!user.connected){
            window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1493&response_type=token&state=Meh&redirect_uri=https://najajason.github.io/naja";
            return;
        }
        if(AutoRunning){
            AutoRunning = false;
...
function getEZSeed(outcome) {
		// csprSeed[0] <-- firefox fix for [object Uint32Array] error
	var csprSeed = window.crypto.getRandomValues(new Uint32Array(1));
	if (csprSeed[0] >= Math.pow(2, 32) || csprSeed[0] < 0) { // foolishly foolproof - right?
		getSeed(outcome);
	} else {
		Dispatcher.sendAction('UPDATE_CLIENT_SEED', {
			str: csprSeed[0].toString()
		});
	$(".seed").text(csprSeed[0].toString());
	}
}
