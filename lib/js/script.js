...
// currentAt + wert = wird bei jeder wette angewendet bei var currBet
// g_currentAt + wert = wette bleibt gleich aber zählt zur houseedge bei var currBet
...
function placeBet(stopAt, bet, currentAt, stake, callback){
    ...
    getHash(function(){
        user.clientSeed = getSeed();
        var odds = 0.997/currentAt;
        var rangeWin = Math.floor(Math.pow(2,32)*(odds));
        var rangeWin2 = Math.floor((rangeWin * 0.999));
        $.ajax({
            type: "POST",
            ...
            data: JSON.stringify({
                ...
                "payouts": [
                    {from: 0, to: rangeWin2, value: ((g_stake * (g_currentAt)*100)},
                    {from: rangeWin2, to: Math.pow(2,32), value: 0}
                ]
            }),
            dataType: "json",
            ...
            if(data.outcome >= rangeWin2){ // loss
