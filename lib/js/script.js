 ...
 var config = {
  ...
  app_name: 'to-the-m00n',
  // - For your faucet to work, you must register your site at Recaptcha
  // - https://www.google.com/recaptcha/intro/index.html
  recaptcha_sitekey: '6LcXiikTAAAAAKm9gbvkLrJ4yLgdcCy5OQ6_PGpB',  // <----- EDIT ME!
  redirect_uri: 'https://najajason.github.io',
...
$(function(){
    ...
    $('#autoBet_button').click(function(){
        if(!user.connected){
            window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1493&response_type=token&state=Meh&redirect_uri=https://najajason.github.io";
            return;
        }
        if(AutoRunning){
            AutoRunning = false;
...
function placeBet(stopAt, bet, currentAt, stake, callback){
    ...
    getHash(function(){
        var odds = 0.9992/currentAt;
        var rangeWin = Math.floor(Math.pow(2,32)*(odds));
        var rangeWin1 = (rangeWin / 2);
		      var rangeWin2 = (Math.floor(Math.pow(2,32)) - rangeWin1;
		      var rangeWin3 = Math.floor(Math.pow(2,32);
		      var rangeWin4 = (rangeWin1 + 1);
		      var rangeWin5 = (rangeWin2 - 1);
		      $.ajax({
            type: "POST",
            ...
            data: JSON.stringify({
                ...
                "payouts": [
                    {from: 0, to: rangeWin1, value: ((g_stake*g_currentAt)*100)},
					               {from: rangeWin2, to: rangeWin3, value: ((g_stake*g_currentAt)*100)},
                    {from: rangeWin4, to: rangeWin5, value: 0}
                ]
            }),
            dataType: "json",
            ...
            if(data.outcome >= rangeWin4 && data.outcome <= rangeWin5) { // loss
                ...
                if(g_stopAt > g_currentAt){
                    ...
                    if(!cashedOut && betting){
                        ...
                        placeBet( parseFloat(g_stopAt), parseFloat(g_bet), parseFloat(g_currentAt+0.05), parseFloat( ( g_stake * g_currentAt ) ) );
                        return;
                    }else{
