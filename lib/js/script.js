...
function onloadCallback() {
    grecaptcha.render('faucetClaimCaptcha', {
        'sitekey' : '6LcXiikTAAAAAKm9gbvkLrJ4yLgdcCy5OQ6_PGpB',
...
$(function(){
    ...
    $('#autoBet_button').click(function(){
        if(!user.connected){
            window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1493&response_type=token&state=Meh&redirect_uri=http://najajason.github.io/najajason";
            return;
        }
        if(AutoRunning){
            AutoRunning = false;
...
$('#bet_cashout_button').click(function(){
    if(!user.connected){
        window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1493&response_type=token&state=Meh&redirect_uri=http://najajason.github.io";
        return;
    }
    if(betting){
        cashedOut = true;
...
function placeBet(stopAt, bet, currentAt, stake, callback){
    ...
    $('#bust_box').html("<span>"+parseFloat((g_stake / g_bet)).formatMoney(2,'.',',')+"</span>x");
    ...
    getHash(function(){
        ...
        $.ajax({
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                $('#bet_input').attr("disabled", false);
                $('#cashout_input').attr("disabled", false);
                $('#bust_box').html("Crashed @ <span>"+parseFloat((g_stake / g_bet)).formatMoney(2,'.',',')+"</span>x");
                $('#bet_cashout_button').text("Bet");
                ...
                if(g_stopAt > g_currentAt){
                    ...
                    if(!cashedOut && betting){
                        if(g_currentAt <= 1.004){
                         
                           console.log("Not cashed out, doing next bet");
                           placeBet( parseFloat(g_stopAt), parseFloat(g_bet), parseFloat(g_currentAt+0.004), parseFloat( ( g_stake * g_currentAt ) ) );
                           return;
                        }else{
                           console.log("Not cashed out, doing next bet");
                           placeBet( parseFloat(g_stopAt), parseFloat(g_bet), parseFloat(g_currentAt), parseFloat( ( g_stake * g_currentAt ) ) );
                           return;
                        };
                    }else{
